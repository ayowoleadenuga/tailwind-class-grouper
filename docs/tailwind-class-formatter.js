...
/**
 * Format grouped classes as an object
 */
function formatAsObject(grouped, indent = '  ') {
  const lines = ['{'];
  ...
  Object.entries(grouped).forEach(([category, classes], index, arr) => {
    ...
    // Add classes
    const classString = `${indent}"${category}": "${classes.join(' ')}"`;
    const isLast = index === arr.length - 1;
    lines.push(classString + (isLast ? '' : ','));
  });
...
function formatTailwindClasses(classString, options = {}) {
  const {
    ...
    minClasses = 4,       // Minimum number of classes before formatting
    indent = '  '         // Indentation string
  } = options;
  
  const classes = classString.split(/\s+/).filter(Boolean);
  
  // Don't format if below threshold
  ...
  switch (format) {
    ...
    case 'object':
      return formatAsObject(grouped, indent);
    case 'clsx':
    default:
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  window.groupTailwindClasses = groupTailwindClasses;
